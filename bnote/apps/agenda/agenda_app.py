...
class AgendaApp(BnoteApp):
        ...
        return ui.UiMenuBar(
            ...
            menu_item_list=[
                ui.UiMenuBar(
                    ...
                    menu_item_list=[
                        ui.UiMenuItem(name=_("&new"), action=self._exec_add_event,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='N'),
                        ui.UiMenuItem(name=_("&modify"), action=self._exec_modify_event),
                        ui.UiMenuItem(name=_("&change status"), action=self._exec_comute_todo,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='T'),
                        ui.UiMenuItem(name=_("&delete"), action=self._exec_delete_event,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_DELETE),
                        ui.UiMenuItem(name=_("delete e&xpired events"), action=self._exec_delete_event_expired,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_DELETE),
                        ui.UiMenuItem(name=_("delete &completed events"), action=self._exec_delete_done),
                    ]
                ),
                ui.UiMenuBar(
                    name=_("&goto"), action=self.goto,
                    menu_item_list=[
                        ui.UiMenuItem(name=_("&previous week"), action=self._previous_week,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_SHIFT_TAB),
                        ui.UiMenuItem(name=_("&next week"), action=self._next_week,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_TAB),
                        ui.UiMenuItem(name=_("previous d&ay"), action=self._previous_day),
                ...
                ui.UiMenuBar(
                    name=_("fi&nd"),
                    menu_item_list=[
                        ui.UiMenuItem(name=_("find &date"), action=self._exec_find_date,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='D'),
                        ui.UiMenuItem(name=_("find in a&genda"), action=self._exec_find_event,
                ...
                ui.UiMenuBar(
                    ...
                    menu_item_list=[
                        ui.UiMenuItem(name=_("&import agenda file"), action=self._exec_import_agenda,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='I'),
                        ui.UiMenuItem(name=_("&export agenda file"), action=self._exec_export_agenda,
    ...
    def error_agenda(self):
        ...
        f.close()
        create_file()
        self._current_dialog=ui.UiInfoDialogBox(message=_("the file has been correctly copy and the agenda has clear"), action=self._exec_cancel_dialog)
    ...
    def hide_item(self):
        ...
        if self.filter=="calendar" or self.filter=="today" or self.filter=="mode" or self.filter=="date":
            self._menu.get_object(self.goto).hide()
        else:
            self._menu.get_object(self.goto).unhide()
        return True
    ...
    def _select_mode(self):
        self.lst=[_("from today's ({})").format(self.standard_presentation(True)), _("for today ({})").format(self._present_today(True)), _("not done ({})").format(self._present_todo(True)), _("all agenda ({})").format(self._present_all(True)), _("calendar {}").format(self.get_name_day(self.get_date(), complete=True))]
        return True
    ...
    def _exec_find_event(self, txt="", criter_index=0, only_not_done=False):
        self._current_dialog=ui.UiDialogBox(
            name=_("find"),
            item_list=[
                ui.UiEditBox(name=_("&element to find"), value=("string character", txt)),
                ui.UiListBox(name=_("cri&terion"), value=("criter", [_("all"), _("subject"), _("content")]), current_index=criter_index),
    ...
    def _exec_import_agenda(self, path=FileManager.get_root_path()):
        ...
        if path==FileManager.get_root_path():
            list_item += [
                ui.UiButton(name=_("usb &key"), action=self._exec_import_agenda,
    ...
    def _exec_export_agenda(self, name="", path=None):
        ...
        self._current_dialog=ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiListBox(name=_("file &location"), value=("location", [_("my documents"), _("choose folder")]), current_index=index_path),
                ui.UiButton(name=_("&next"), action=self._exec_export_next,
    ...
    def _exec_export_next(self, name="", path=None, first=True):
        ...
        if not first and kwargs['location']==_("my documents"):
            self._current_dialog=ui.UiMessageDialogBox(
                name=_("information"),
                message=_("the file will be exported to the my documents folder. Do you want to continue?"),
                buttons=[
                    ui.UiButton(name=_("&yes"), action=self._exec_valid_export,
                            action_param={'name': name, 'path': FileManager.get_root_path()}),
                    ui.UiButton(name=_("&no"), action=self._exec_export_agenda,
        ...
        lst=[
            ...
            ui.UiButton(name=_("&export"), action=self._exec_valid_export,
                     action_param={'name': name}),
            ui.UiButton(name=_("&back"), action=self._exec_export_agenda,
                     action_param={'name': name, 'path': True}),
            ui.UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
        ]
        if path==FileManager.get_root_path():
            lst+=[
                ui.UiButton(name=_("usb &key"), action=self._exec_export_next,
                        action_param={'name': name, 'path': FileManager.get_usb_flash_drive_path()}),
            ]
        elif path==FileManager.get_usb_flash_drive_path():
            lst+=[
                ui.UiButton(name=_("my &documents"), action=self._exec_export_next,
                        action_param={'name': name, 'path': FileManager.get_root_path()}),
            ]
        else:
            lst+=[
    ...
    def _exec_about(self):
        self._current_dialog=ui.UiMessageDialogBox(
            name=_("about agenda"),
            message=_("agenda, version {}, copyright (C){} Theotime Berthod.\nIn collaboration with Eurobraille.\nFor suggestions or problems, contact Eurobraille or directly the application developer to {}.").format(self.version,datetime.datetime.now().year,"acres.louer.0l@icloud.com"),
            buttons=[
    ...
    def _exec_valid_add_event(self):
        ...
        if test!=True:
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                message=test,
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_add_event,
    ...
    def _exec_valid_modification(self):
        ...
        if test!=True:
            self._current_dialog=ui.UiMessageDialogBox(
                ...
                message=test,
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_modify_event,
    ...
    def _exec_valid_find_date(self):
        ...
        if test!=True:
            self._current_dialog=ui.UiMessageDialogBox(
                ...
                message=test,
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_find_date,
    ...
    def _exec_valid_find_element(self):
        ...
        if test:
            ...
            self._current_dialog=ui.UiInfoDialogBox(message=number, action=self._exec_cancel_dialog)
    ...
    def _exec_valid_export(self, name, path=None):
        if not path:
            ...
            path=kwargs['file']
            if not path:
                self._current_dialog=ui.UiInfoDialogBox(message=_("select folder"), action=self._exec_export_agenda,
                                                     action_param={'name': name})
                return
        exporter=edition.export_file(name, path)
        if exporter==True:
            log.info("success file exported")
            self._current_dialog=ui.UiInfoDialogBox(message=_("the file has been correctly exported in {} folder").format(path), action=self._exec_cancel_dialog)
        elif exporter=="exist":
            ...
            self._current_dialog=ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_export_agenda,
            ...
            self._current_dialog=ui.UiInfoDialogBox(
                name=_("warning"),
                message=_("error during the exportation, verify your agenda and try again"),
    ...
    def close_after_same_day(self):
        if Settings().data['agenda']['remember']=="tomorrow" or Settings().data['agenda']['remember']=="same_tomorrow":
    ...
    def get_name_day(self, date, complete=False):
        ...
        if complete==True:
            return name_day_dict[date_name] + " " + date_found[0] + " " + month_dict[date_found[1]] + " " + date_found[2]
